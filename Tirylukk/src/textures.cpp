...
SDL_Texture * texture_class::load_texture(const char * file_name) {
	...
	SDL_Texture * texture = SDL_CreateTextureFromSurface(engine_class::renderer, temp_surface);
...
void texture_class::draw_texture(SDL_Texture* text, SDL_Rect src, SDL_Rect dest, SDL_RendererFlip flip_flag) {
	...
	SDL_RenderCopyEx(engine_class::renderer, text, &src, &dest, NULL, NULL, flip_flag); //nulls are for rotation angle and pivot point which are none for now
}